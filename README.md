...
supabase/sql/  DB 마이그레이션(테이블/RPC/RLS/트리거). Supabase SQL Editor에서
               001 → 006 순서대로 실행. 999는 샘플 데이터용 수동 스크립트.
```
...
1. **쓰기는 무조건 RPC만.** `create_trial` / `respond_to_trial` / `place_bet` /
   `add_comment` / `update_my_profile` / `mark_notifications_read` /
   `mark_all_notifications_read`. 테이블에 `.insert()` / `.update()` 하는 코드는
   어디에도 없습니다 (RLS로 차단, `supabase/sql/006_rls_lockdown.sql`).
...
초대 링크 공유 방식은 없앴습니다. 사연 작성 화면에서 **상대방 닉네임**을 입력하면
`create_trial` RPC가 그 닉네임의 유저를 피고로 지정하고, 피고에게 "OO님이 재판을
요청했습니다" 알림을 보냅니다(홈 화면 상단 배너 + 종 아이콘 알림함). 원고는 생성 직후
"피고에게 요청하였습니다" 메시지만 보고 대기 화면으로 이동합니다. 피고는 홈 화면 배너나
알림함에서 눌러 `ConsentRequestScreen`으로 들어가 수락/거절합니다.
...
DB 트리거(`supabase/sql/004_notification_triggers.sql`)가 담당합니다.
...
- `DEMO_MODE = false` (현재값): 실제 Supabase 백엔드에 연결됩니다. 로그인이 필요하고,
  ...
  성공 시 500 P-COIN이 자동 지급됩니다(`005_signup_bonus.sql`).
