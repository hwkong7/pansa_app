...
// ── 읽기: 특정 재판의 댓글 목록 (오래된 순, limit개까지) ─────────────
// 댓글이 많은 인기 재판에서 한 번에 수백 개를 다 불러오지 않도록, "더보기"를 누를 때마다
// limit을 COMMENTS_PAGE_SIZE만큼 늘려 다시 부르는 방식(단순하고 항상 일관된 결과).
export async function listComments(trialId: number, limit = COMMENTS_PAGE_SIZE): Promise<Comment[]> {
