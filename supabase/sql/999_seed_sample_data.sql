-- [백엔드 담당자 요청/협의용] 999_seed_sample_data.sql (수동 실행 전용, 낮은 우선순위)
--
-- "더미는 없애되, 사연+댓글+투표 현황은 몇 개 있어야 함"을 위한 실 DB 샘플
-- 데이터. 프론트 데모 데이터를 지우는 대신, 실제 계정으로 만든 샘플 재판을
-- DB에 심어둔다.
--
-- ⚠️ 이 파일은 trials/comments/bets 테이블에 RPC 없이 직접 insert하므로,
--    실제 컬럼명을 백엔드 담당자와 먼저 맞춰본 뒤 그쪽에서 실행하는 걸 권장.
--
-- 실행 전 준비:
...
--      직접 insert 하지만(관리자 권한이므로 RLS는 우회됨), 003의 트리거가 이미
--      적용돼 있으면 comment_count/알림도 함께 채워진다.
